...
namespace WardingWins
{
    public class WardDatabase
    {
        ...
        {
            WardDatabase wardItem = obj as WardDatabase;
        ...
        public static void DrawWardSpots()
        {
            foreach (Vector3 wardPos in WardSpots)
            {
                ...
                if (IsOnScreen(screenPos[0], screenPos[1]) && ObjectManager.Player.Distance(wardPos) < Program.WardingWins["drawDistance"].Cast<Slider>().CurrentValue)
                {
        ...
        public static void DrawSafeWardSpots()
        {
            foreach (WardSpot safeWardSpot in SafeWardSpots)
            {
                ...
                if (IsOnScreen(screenPos[0], screenPos[1]) && ObjectManager.Player.Distance(safeWardSpot.MagneticPosition) < Program.WardingWins["drawDistance"].Cast<Slider>().CurrentValue)
                {
